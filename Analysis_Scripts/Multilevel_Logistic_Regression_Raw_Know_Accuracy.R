...
SESSION2_FILTERED = 
  ...
  mutate(
    ...
    formatted_response_confidence_RKN = factor(formatted_response_confidence_RKN)) %>%
  mutate(K_hit_binary = ifelse((correct_coded == "hit" & formatted_response_confidence_RKN == "K"),1,0))
...
# test random effects in following 4 models
mod2 = glmer(K_hit_binary ~ 
                prior_condition_combo*GROUP*DELAY +
...
# test for the significance of the 3 way interaction
mod2A = glmer(K_hit_binary ~ 
                prior_condition_combo + GROUP + DELAY +
...
# test for the significance of all 3 2-way interactions, combined
mod2E = glmer(K_hit_binary ~ 
                 prior_condition_combo + GROUP + DELAY +
                 (1 + prior_condition_combo | subjectID) + (1 | formatted_stimulus), 
               data=SESSION2_FILTERED,
               family = binomial, 
...
# test main effects in following three models
mod2F = glmer(K_hit_binary ~ 
...
# potential main effect of group model
mod2I = glmer(K_hit_binary ~ 
                 GROUP +
                 (1 + prior_condition_combo | subjectID) + (1 | formatted_stimulus), 
               data=SESSION2_FILTERED,
               family = binomial, 
...
# probability K hit younger, collapsed across delay / prior condition
younger = fixef(mod2I)["(Intercept)"]
round(exp(younger) / (1+exp(younger)),2) 

# probability K hit older, collapsed across delay / prior condition
older = fixef(mod2I)["(Intercept)"] + fixef(mod2I)["GROUPolder"]
round(exp(older) / (1+exp(older)),2) 

# obtain means
SESSION2_FILTERED %>%
  group_by(GROUP) %>%
  summarise(mean = round(mean(K_hit_binary,na.rm=TRUE),2))
